...
const CameraFeed = ({
  ...
  const initializePoseDetection = async () => {
    try {
      ...
      if (!isPushUpsSelected && !isPlankSelected && !isSquatSelected && !isLungesSelected && !isBurpeesSelected && !isJumpingJacksSelected && !isSidePlankSelected && !isHighKneesSelected) {
        return;
      }
  ...
  useEffect(() => {
    ...
    const runFrame = async () => {
      if (
        ...
        videoRef.current &&
        (isPushUpsSelected || isPlankSelected || isSquatSelected || isLungesSelected || isBurpeesSelected || isJumpingJacksSelected || isSidePlankSelected || isHighKneesSelected)
      ) {
  ...
  useEffect(() => {
    if (poseDetectionRef.current && (isPushUpsSelected || isPlankSelected || isSquatSelected || isLungesSelected || isBurpeesSelected || isJumpingJacksSelected || isSidePlankSelected || isHighKneesSelected)) {
      poseDetectionRef.current.setExerciseMode(
        isPlankSelected ? 'plank' :
        isSquatSelected ? 'squats' :
        isLungesSelected ? 'lunges' :
  ...
  {(isPushUpsSelected || isPlankSelected || isSquatSelected || isLungesSelected || isMountainClimbersSelected || isJumpingJacksSelected || isSidePlankSelected || isHighKneesSelected) && isActive && (
